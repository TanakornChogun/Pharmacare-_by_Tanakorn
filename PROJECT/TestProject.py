...
# path ของฐานข้อมูล ( C:\Users\Lenovo\OneDrive\เดสก์ท็อป\PROJECT\pharmacy.db)
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------#
    ...
    def _reset_session(self):
        ...
        # ถ้ามี textbox สรุปตะกร้าอยู่แล้วให้เคลียร์
    ...
    #  สร้างตะกร้าถ้ายังไม่มี (กัน None/ไม่ได้สร้าง)
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------#
    ...
    # ล้างหน้าและขึ้นพื้นหลัง
        ...
        # แสดงค่าขนส่ง / VAT  / รวมสุดท้าย (ส่งฟรีถ้า >=2 ชิ้น หรือ >=2 รายการ)
    ...
    # อ่าน input (อ่านให้ครบ *ก่อน* เปลี่ยนหน้า เพื่อลด invalid command name)
        ...
        # ไปหน้า QR/อัปสลิป ด้วย order_id เดิม 
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------#
    ...
    # ต้องมีสลิป
        ...
        try:
            ...
            # 3) อัปเดตออเดอร์เดิม → แนบสลิป + ปรับเป็น submitted 
            c.execute(
        ...
        # 5) เคลียร์ตะกร้า + แจ้งผล + ไปหน้า History/หลัก 
        self.cart.clear()
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------#
    ...
    def _choose_profile_image(self):
        ...
        try:
            ...
            im = ImageOps.fit(im, (150, 150), Image.LANCZOS)  
...
#   (เรียกใช้ได้ซ้ำ ปลอดภัย)
    ...
    # ─────────────────────────────────────────────────────────────
    #  คำนวณช่วงเวลา + ป้ายบอกช่วง + ฐานสำหรับปุ่มเลื่อนก่อนหน้า/ถัดไป
    ...
    def _query_sales_summary(self, start_iso: str, end_iso: str):
        ...
        # รวมเป็นยอดต่อบิล (คิด VAT 7% ต่อบิล แล้วค่อย SUM)
        c.execute(f"""
            WITH per_order AS (
                SELECT 
                    o.id,
    ...
    # ─────────────────────────────────────────────────────────────
    #  UI รายงานสรุปยอดขาย (มี Back, Export, ปฏิทิน, ปุ่มเลื่อนช่วง)
...
def send_email(to_addr: str, subject: str, body: str) -> bool:
    ...
    SMTP_PORT = 587
    SMTP_USER = "tanakorn.ja@kkumail.com"   
    SMTP_PASS = "acot riqp snpw jepx"        
